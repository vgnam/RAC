...
class HystereticQLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        mac_out = th.stack(mac_out, dim=1)  # Concat over time, (bs, max_seq_length, n_agents, n_actions)

        # Pick the Q-Values for the actions taken by each agent, (bs, max_seq_length-1, n_agents)
        ...
        target_mac_out = th.stack(target_mac_out[1:], dim=1)  # Concat across time, (bs, max_seq_length-1, n_agents, n_actions)
        ...
        if self.args.double_q:
            ...
            target_max_qvals = th.gather(target_mac_out, 3, cur_max_actions).squeeze(3) # (bs, max_seq_length-1, n_agents)
        else:
        ...
        assert N == 1, "Use N==1 for fair comparison."
        if N == 1:
        ...
        # Td-error
        td_error = (targets.detach() - chosen_action_qvals)     # targets.shape=(bs, max_seq_length-1, n_agents)
        mask = mask.expand_as(td_error)
        # 0-out the targets that came from padded data
        masked_td_error = td_error * mask
        square_td_error = masked_td_error ** 2

        # For samples with negative TD-error, multiply it by a weight <beta> to reduce its impact
        square_td_error[td_error <= 0] *= self.args.beta

        # Normal L2 loss, take mean over actual data
        loss = square_td_error.sum() / mask.sum()
