...
class QLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        assert N==1, "We set N to 1 for fair comparison by default."
        if N == 1:
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            self.logger.log_stat("episode_return_max", episode_returns.max().item(), t_env)
            self.logger.log_stat("episode_return_min", episode_returns.min().item(), t_env)
            self.logger.log_stat("step_rew_max", batch_rewards.max().item(), t_env)
            self.logger.log_stat("step_rew_min", batch_rewards.min().item(), t_env)
